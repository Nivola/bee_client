...
class CmpAuthService(CmpBaseService):
    ...
    def create_token(self, api_user=None, api_user_pwd=None, api_user_secret=None, headers=None):
        ...
        :raises CmpApiClientError: raise :class:`CmpApiClientError`
        """
        res = self.manager.client.create_token(api_user, api_user_pwd, api_user_secret, headers)
        return res
    ...
    def exist_role(self, name):
        ...
        uri = "/v1.0/nas/roles"
        roles = self.manager.api_request("auth", uri, "GET", data, silent=True).get("roles")
        res = None
        if len(roles) > 0:
            res = roles[0]
