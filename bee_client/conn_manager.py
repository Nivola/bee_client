...
def http_request(method):
    def inner(ref, *args, **kwargs):
        ...
        except CmpApiConnectionManagerError as exc:
            ref.logger.error(exc.value, exc_info=True)
            raise
        except Exception as exc:
            ref.logger.error(exc, exc_info=True)
            raise CmpApiConnectionManagerError(str(exc)) from exc
        return res
...
class CmpApiConnectionManagerError(Exception):
    ...
    def __repr__(self):
        return f"CmpConnectionManagerError: {self.value}"

    def __str__(self):
        return f"{self.value}, {self.code}"


class CmpApiConnectionManager():
...
class CmpApiConnectionManagerAbstractPlugin():
    ...
    def __setup_headers(self):
        self.headers["Authorization"] = f"Bearer {self.manager.token}"
        return self.headers
    ...
    def get_uri(self, uri):
        return f"{self.manager.endpoint}/{uri}"
...
class CmpApiConnectionManagerPluginDb(CmpApiConnectionManagerAbstractPlugin):
    ...
    def setup(self, engine, host, pwd, **kwargs):
        ...
        if engine not in self.SUPPORTED_ENGINE:
            raise CmpApiConnectionManagerError(f"supported engine are {self.SUPPORTED_ENGINE}")
    ...
    def get_schemas(self):
        ...
        self.logger.debug("get database schemas: %s", res)
        return res
    ...
    def get_tables(self, schema_name):
        ...
        self.logger.debug("get database schema %s tables: %s", schema_name, res)
        return res
    ...
    def get_users(self):
        ...
        self.logger.debug("get database users: %s", res)
        return res
